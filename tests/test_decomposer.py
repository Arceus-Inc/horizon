...
def test_decompose_persists_team_shape_and_staffing_requirements(tmp_path):
    text = json.dumps(
        {
            "goals": [
                {
                    "title": "Ship the coordinated launch",
                    "metric": "launch complete",
                    "target": "one verified release",
    ...
    decomposer, decisions, strategy, *_ = _decomposer(tmp_path, text)
    decisions.put(Decision(id="dec_1", statement="coordinate launch"))

    (goal,) = decomposer.decompose("dec_1")
    record = strategy.get(goal.id)
...
def test_decompose_defaults_legacy_goal_shape_to_single(tmp_path):
    text = json.dumps({"goals": [{"title": "Ship one deliverable", "score": 0.8}]})
    decomposer, decisions, strategy, *_ = _decomposer(tmp_path, text)
    decisions.put(Decision(id="dec_1", statement="ship"))
